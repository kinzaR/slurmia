...
router.post('/create', function (req, res, next) {
    ...
    //console.log("the req.query is : ");
    //console.log(util.inspect(req._user.home, {showHidden: false, depth: null}))
    ...
    for (var name in jobConfig.options) {
        ...
        if (option.type === 'file' && option.mode === 'id') { //if you use the file's form inputs
            fileIdsFromJobConfig.push(option.value);
            //if you use the form fils
            //console.log("++++fileIdsFromJobConfig++++" + option.value +"------fin-----");
    ...
    async.waterfall([
        ...
        function (cb) {
            ...
            job.createJobFolder(folderName, req._parent, req._user, function (err) {
                console.log("creating a Job Folder ....");
                if (err) {
                    console.log(err)
                    cb(err)
                } else {
...
//added by Kinza
                //console.log(util.inspect(computedOptions, {showHidden: false, depth: null}));
                //console.log(" *********************************" );
// fin

            var commandLine = "'" + path.join(config.steviaDir, config.toolsPath, tool, executable) + "' " + computedOptions.join(" ");
//added by kinza
                //console.log("commandLine: "+ util.inspect(commandLine, {showHidden: false, depth: null}));

//fin

            var commandQsub = path.join(realOutPath, ".command.qsub.sh");
            //kinza
            console.log("\n***commandQsub: \n"+ util.inspect(commandQsub, {showHidden: false, depth: null}) +"\n*********fin******");
//fin
            fs.writeFile(commandQsub, "#!/bin/bash\n" + commandLine, function (err) {
                if (err) {
                    cb('Could not create ' + commandQsub);
                } else {
                    //Kinza task: i have to change this Command Line
                    //var command = "qsub -N '" + job.qId + "' -q '" + config.queue + "' -j y -o '" + path.join(realOutPath, ".out.job") + "' '" + commandQsub + "'";

                    var command = "sbatch  --job-name '" + job.qId + "'  --mem=3000   --output '" + path.join(realOutPath, ".out.job") + "' '" + commandQsub + "'";
            ...
            fs.writeFile(commandQsub, "#!/bin/bash\n" + commandLine, function (err) {
                if (err) {
                    ...
                    if (jobConfig.disableQueue === true) {
                      ...
                      console.log("i am here pahact");
                        // command = "qsub -sync y -q '" + config.queue + "' -j y -o '" + outFile + "' '" + commandQsub + "'";
                        //command = "qsub -sync y -q '" + config.queue + "' -j y -o '" + outFile + "' '" + commandQsub + "'";
                        command = "srun   --mem=3000  --output '" + outFile +"' '" + commandQsub + "'";
                        console.log(command);
...
router.get('/example', function (req, res, next) {
    ...
    //console.log("i am here metabolizer :P ");
    async.waterfall([
        function (cb) {
